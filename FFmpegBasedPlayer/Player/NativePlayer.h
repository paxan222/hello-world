...
extern "C"
{
	...
	typedef void(CALLBACK *FErrorCallback)(std::string error);
	...
	NATIVEPLAYER_API PVOID WINAPI GetLivePlayer(VendorType wVendorType, FDecodeCallback fDecodeCallback, FEndInitCallback fEndInitCallback, DWORD openStreamMode, HWND hMainWindow = NULL);

	// Получить указатель на Rtsp плеер

...
NATIVEPLAYER_API PVOID WINAPI GetPlayer(PCHAR pchFileName, VendorType wVendorType, FDecodeCallback fDecodeCallback, FFileEndCallback fFileEndCallback, HWND hMainWindow)
{
	...
	case VT_UNKN:
	{
		pPlayer = new CFFmpegPlayer(pchFileName, fDecodeCallback, fFileEndCallback, nullptr, nullptr, NULL, hMainWindow);
		break;
	}
	case VT_HIK_VISION:
...
NATIVEPLAYER_API PVOID WINAPI GetLivePlayer(VendorType wVendorType, FDecodeCallback fDecodeCallback, FEndInitCallback fEndInitCallback, DWORD openStreamMode, HWND hMainWindow)
{
	CBasePlayer *pPlayer = nullptr;
...
// Получаем указатель на rtsp-плеер
NATIVEPLAYER_API PVOID WINAPI GetRtspPlayer(PCHAR rtspPath, FDecodeCallback fDecodeCallback, FFileEndCallback fFileEndCallback, FErrorCallback fErrorCalllback, int timeout, HWND hMainWindow)
{
	//_asm int 3
	CBasePlayer *pPlayer;
	pPlayer = new CFFmpegPlayer(rtspPath, fDecodeCallback, fFileEndCallback, nullptr, fErrorCalllback, timeout, hMainWindow);

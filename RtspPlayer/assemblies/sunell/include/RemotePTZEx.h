#ifndef _REMOTEPTZEX_H_
#define _REMOTEPTZEX_H_
...
public:
	...
	*		
	*************************************************************************/
	RemotePTZEx(const DeviceInfo &p_objDeviceInfo, const int p_nCameraID);

	RemotePTZEx(const DeviceInfoEx &p_objDeviceInfoEx, const int p_nCameraID);
...
public:
	...
	*		操作云台向上旋转
	**输入：
	*		p_nSpeedValue：云台向上旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向下旋转
	**输入：
	*		p_nSpeedValue：云台向下旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向左旋转
	**输入：
	*		p_nSpeedValue：云台向左旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向右旋转
	**输入：
	*		p_nSpeedValue：云台向右旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向左上旋转
	**输入：
	*		p_nSpeedValue：云台向左上旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向左下旋转
	**输入：
	*		p_nSpeedValue：云台向左下旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向右上旋转
	**输入：
	*		p_nSpeedValue：云台向右上旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*		操作云台向右下旋转
	**输入：
	*		p_nSpeedValue：云台向右下旋转的速度值(范围：1--63)
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*************************************************************************/
	...
	/************************************************************************
	**概述：
	*		巡游预置点设置，该方法的调用应在setTourStartPoint之后，setTourEndPoint之前。
	**输入：
	*		p_nPresetValue：预置点编号（1-128）
			p_nSpeedValue：巡游速度值（0-7），0为最快速度(此参数为预留参数)。
			p_nTimeValue：巡游到每个预置点停留时间值（0Sec - 255Sec）
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	*		若方法失败则返回小于0的值，可以通过ErrorInfo::formatMessage来获取错误信息。
	*************************************************************************/
	int addTourPoint(const int p_nPresetValue, const int p_nSpeedValue, const int p_nTimeValue);
	...
	*		若方法失败则返回小于0的值，可以通过ErrorInfo::formatMessage来获取错误信息。
	*************************************************************************/
	int setTourEndPoint(const int p_nValue);
	...
	*		p_nTypeValue：执行看守位的类型（1：预置位，2：扫描，3：自学习，4：巡游）。
			...
			p_nTimeValue：执行看守位时间，以分为单位（范围：1-240分钟），即云台最后一次执行操作过p_nTimeValue时间后执行看守位动作，
	**输出：
	*		无
	**返回值：
	*		若方法成功，返回SN_SUCCESS；
	...
	*			setPostion(POSTION_TYPE_PAN|POSTION_TYPE_TILE，100.00， 50.00，1);
	*************************************************************************/
	...
	*		设置云台心跳接收超时过滤标志
	**输入：
	*		p_bHeartReceiveTimeoutFilterFlag：云台心跳接收超时过滤标志
	*		
	**输出：
	*		无
	**返回值：
	...
	*
	*备注：	
			此接口为兼容不支持PTZ位置获取命令设备（例如IPC8180设备，可以通过RemoteSystem::getSystemInfo获取设备硬件模组类型）。
			如果设备不支持PTZ位置获取命令设备，调用此接口设置标志为true。
			当不调用此接口时，默认为对PTZ云台心跳不进行过滤；当设置为true时，过滤云台心跳接收超信息。
	*************************************************************************/
	int setHeartReceiveTimeoutFilterFlag(const bool& p_bHeartReceiveTimeoutFilterFlag);

private:
	int sendPTZOperation(std::string p_objPTZOperation, bool p_bReturnValue = false);
	/*int sendOperation(std::string p_objPTZOperation, bool p_bReturnValue);*/

	//拷贝构造函数
	RemotePTZEx(const RemotePTZEx& p_objObject)
	{
	}

	//赋值函数
	RemotePTZEx& operator = (const RemotePTZEx& p_objObject)
	{
		return *this;
	}
...
private:
	...
	std::string				m_strOpreationReturnValue;

	PTZOperateManager*	m_pobjPTZOperateManager;

...
public:
	...
	*		设置TopX
    **输入：
	*		p_nTopX:整形值，表示水印信息在工作区左上角 x 的坐标位置
	**输出：
	*		无
	**返回值：
	*		无 
	...
	*		设置TopY
    **输入：
	*		p_nTopY:整形值，表示水印信息在工作区左上角 y 的坐标位置
	**输出：
	*		无
	**返回值：
	*		无 
	...
	*		设置OSD信息
    **输入：
	*		p_pszInfo：字符串，最大长度为CONST_MAXLENGTH_OSDINFO字节,结尾以‘\0’结束。
	**输出：
	*		无
	**返回值：
	*		true： 成功
	*		false：失败 
	**功能：
	*		若输入p_pszInfo长度<=CONST_MAXLENGTH_OSDINFO，返回true，并保存到m_szInfo;
	*		若输入p_pszInfo长度>CONST_MAXLENGTH_OSDINFO，返回false
	*****************************************************************************/
...
private:
	...
	int		m_nTopX;								//水印信息在工作区左上角 x 的坐标
	int		m_nTopY;								//水印信息在工作区左上角 y 的坐标

#ifndef _REMOTEPLAYBACK_H_
#define _REMOTEPLAYBACK_H_

//#include "ILivePlayback.h"
...
class SN_DLL_API RemotePlayback
{
public:
	RemotePlayback();
	RemotePlayback(const DeviceInfo &p_objDeviceInfo, const char * p_pszFileName);		//此接口废弃
	RemotePlayback(const DeviceInfoEx &p_objDeviceInfoEx, const char * p_pszFileName); //此接口废弃
	RemotePlayback(const DeviceInfoEx &p_objDeviceInfoEx, const RecordInfo & p_objRecordInfo);
	RemotePlayback(const DeviceInfoEx &p_objDeviceInfoEx, const RecordFileEx & p_objRecordFile);
...
public:
	...
	*		此接口废弃
	*************************************************************************/
	void	setPlayFileName(const char* p_pszFileName);
	...
	*		当用不带参数的构造函数构造类时，在open之前必须调用该方法
	*************************************************************************/
	void	setFtpFile(const RecordFileEx & p_objFtpRecordFile);
...
public:
	...
	**输入：
	*		p_bAutoResizeFlag：true：	自动随窗口改变
	*						   false：	不改变
	**输出：
	*		无
	**返回值：
	...
	**输入：
	*		p_hNotifyWnd：	指定接收消息的窗口句柄。RemotePlayback将发送消息到该句柄指定的窗口。若为NULL则表示不接收消息。
	*		
	*		p_nMessage：	指定Windows窗口消息ID。RemotePlayback将发送该消息到由p_hNotifyWnd指定的窗口。这个Windows窗口消息由用户自定义，
	*						Windows	下的WM_USER常量可以帮助用户为Windows窗口定义私有消息，定义方法如WM_USER+X, X是一个整型值。
	*		
	*		p_pParam：		指定发送消息时的携带的消息参数。该消息参数并不被RemotePlayback改变，RemotePlayback发送消息时只是简单地
	*						将该消息参数又发回给窗口。
	**输出：
	*		无
	**返回值：
	...
	*************************************************************************/
	...
	/************************************************************************
	**概述：
	*		设置RemotePlayback和网络视频设备之间的网络传输协议。
	**输入：
...
public:
	...
	*		若失败则返回小于0的值，可以通过ErrorInfo::formatMessage来获取错误信息。
	*************************************************************************/
	int	setCurrentFrameNum(const long p_nFrameNum);
...
private:
	...
	************************************************************************/
	...
	//拷贝构造函数
	RemotePlayback(const RemotePlayback& p_objObject)
	{
	}

	//赋值函数
	RemotePlayback& operator = (const RemotePlayback& p_objObject)
	{
		return *this;
	}

private:
	//ILivePlayback *			m_pobjLivePlayback;							//实时回放对象
	...
	char					m_szFileName[CONST_MAXLENGTH_FILE_NAME];	//播放文件名

	RecordInfo				m_objRecordInfo;
	RecordFileEx			m_objRecordFileEx;

	int						m_nTransferProtocol;						//传输协议

	bool					m_bAutoConnectFlag;							//音频连接状态
	
	unsigned	int			m_nNotifyWnd;								//视频窗口句柄
	unsigned    int			m_nMessage;									//指定Windows窗口消息ID
	void*					m_pParam;									//发送消息时的携带的消息参数

	unsigned    int			m_nTryConnectCount;							//尝试连接次数
	...
	RemoteSNAVFilePlayer    *m_pobjRemoteSNAVFilePlayer;
